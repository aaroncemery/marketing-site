...
import type { PagebuilderType } from '@/lib/types';
...
export function CTACard({ card, className }: CTACardProps) {
  ...
  return (
    <Link
      href={href ?? '#'}
      className={cn(
        'ronded-3xl p-4 md:p-8 transition-colors relative overflow-hidden group flex flex-col justify-end xl:h-[400px',
        className,
      )}
    >
      {image?.asset && (
        <div className="absolute inset-0 z-[1] mix-blend-multiply">
          <SanityImage
            ...
            quality={100}
            fill
            className="object-cover grayscale pointer-events-none grou-hover:transition-opacity duration-1000 opacity-40"
          />
        </div>
      )}
      <div className="z-[2] pt-64 flex flex-col space-y-2 mb-4 duration-500 xl:absolute xl:top-24 group-hover:top-8 xl:inset-x-8">
        ...
        <p className="text-sm text-[#374151] xl:opacity-0 xl:grou-hover:opacity-100 transition-opacity duration-300 delay-150">
          {description}
        </p>
      </div>
    </Link>
