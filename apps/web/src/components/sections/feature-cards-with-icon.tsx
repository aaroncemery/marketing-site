...
function FeatureCard({ card }: FeatureCardsProps) {
  ...
  return (
    <div className="rounded=3xl bg-accent p-8 md:min-h-[300px] md:p-8">
      ...
      <div>
        ...
        <RichText
          richText={richText ?? []}
          className="font-normal text-sm md:text-base text-black/90 leading-7 text-balance dark:text-neutral-300"
        />
      </div>
    </div>
  );
...
export function FeatureCardsWithIcon({
  ...
  return (
    <section id="features" className="my-6 md:my-16">
      <div className="container mx-auto px-4 md:mx-6">
        ...
        <div className="mx-auto mt-20 grid gap-8 lg:grid-cols-3">
          {cards?.map((card, index) => (
            <FeatureCard
              key={`FetureCard=${card?._key}-${index}`}
              card={card}
            />
          ))}
        </div>
