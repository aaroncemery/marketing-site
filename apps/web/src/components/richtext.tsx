...
const components: Partial<PortableTextReactComponents> = {
  block: {
    h2: ({ children, value }) => {
      ...
      return (
        <h2
          id={slug}
          className="scroll-mt-20 border-b pb-2 text-3xl font-semibold first:mt-0"
        >
          {children}
        </h2>
      );
    ...
    h3: ({ children, value }) => {
      const slug = parseChildrenToSlug(value.children);
      return (
        <h3 id={slug} className="scroll-m-20 pb-2 text-2xl font-semibold">
          {children}
        </h3>
      );
    },
    h4: ({ children, value }) => {
      const slug = parseChildrenToSlug(value.children);
      return (
        <h4 id={slug} className="scroll-m-20 pb-2 text-xl font-semibold">
          {children}
        </h4>
      );
    },
  ...
  marks: {
    code: ({ children }) => (
      <code className="rounded-md border border-white/10 bg-opacity-3 p-1 text-sm lg:whitespace-nowrap">
        {children}
      </code>
    ),
